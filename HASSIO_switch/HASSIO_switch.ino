...
void setup() {
  //initialize the switch as an output and set to HIGH (off as relys on on/LOW)
...
//      clientName += macToStr(mac);
        String clientName = "Uplighter_switch";

      //if connected, subscribe to the topic(s) we want to be notified about
      //EJ: Delete "mqtt_username", and "mqtt_password" here if you are not using any 
      //if (client.connect((char*) clientName.c_str(),"mqtt", "mqtt")) {  //EJ: Update accordingly with your MQTT account 
        if (client.connect("Uplighter_switch")) {
